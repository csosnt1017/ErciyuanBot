...
import com.example.image.HttpClientFactory;
import com.example.util.PixivUtil;
...
//        demo.privateMsg(0, 10001, 2234567819L, "gkd1[CQ:image,file=3F05368976AB9A9B62187A14ABB693FC1.jpg]2[CQ:image,file=3F05368976AB9A9B62187A14ABB693FC1.jpg][CQ:image,file=3F05368976AB9A9B62187A14ABB693FC2.jpg][CQ:image,file=3F05368976AB9A9B62187A14ABB693FC3.jpg]", 0);
//        demo.privateMsg(0, 10002, 2222222224L, "gkd1[CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg][CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg]2[CQ:image,file=3F05368976AB9A9B62187A14ABB693FC.jpg]", 0);
//        demo.privateMsg(0, 10003, 2111111334L, "gkd1[CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg][CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg]2[CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg][CQ:image,file=3F05368976AB9A9B62187A14ABB693FB.jpg]", 0);
//        demo.privateMsg(0, 10004, 3111111114L, "#南昌天气", 0);
//        demo.privateMsg(0, 10005, 3333333334L, "gkd  1 测试二2 测试内容二 ", 0);
        // 模拟群聊消息
        // 开始模拟群聊消息
        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "p站今日插画排行1", 0);
//        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "给我整#色图", 0);
//        demo.groupMsg(0, 10008, 3456789012L, 11111111114L, "", "[CQ:image,file=3F05368976AB9A9B62187A14ABB693FC1.jpg]", 0);
//        demo.groupMsg(0, 10009, 427984429L, 3333333334L, "", "#二次元", 0);
        // ......
    ...
    public int startup() {
        ...
        DataSourceConfig.getSession();
        StatementWarehouseUtil.initAll();
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        // 这里处理消息
        String returnMsg = StatementWarehouseUtil.insertWarehouse(msg, fromQQ);
        if (returnMsg == null) {
            returnMsg = StatementWarehouseUtil.selectWarehouse(msg);
        }
        if (returnMsg == null) {
            returnMsg = "喵帕斯~";
        }
        CQ.sendPrivateMsg(fromQQ, returnMsg);
        return MSG_IGNORE;
    }
    ...
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
        ...
        if (PixivUtil.checkPixiv(msg)) {
            boolean pass = true;
            if (msg.contains("p站r18")) {
                if (fromGroup != 197446621 && CQ.getGroupMemberInfoV2(fromGroup, fromQQ).getAuthority() == 1) {
                    CQ.sendGroupMsg(fromGroup, "你没有这个权限嗷");
                    pass = false;
                }
            }
            if(pass){
                CQ.sendGroupMsg(fromGroup, "喵帕斯正在搜索图片，请稍后~");
                PixivUtil.listImg(msg).forEach(img -> {
                    CQ.sendGroupMsg(fromGroup, img);
                });
            }
        }
        List<String> imgs = (new CoolQCode(msg)).gets("image", "file");
        imgs.forEach(i -> {
            String image = CC.image(i);
            String img = StatementWarehouseUtil.selectWarehouseByImg(image);
            if (img != null) {
                CQ.sendGroupMsg(fromGroup, img);
            }
        });
        ...
        String returnMsg = StatementWarehouseUtil.selectBan(msg);
        if (returnMsg != null) {
            stringBuffer.append(CC.at(fromQQ)).append(returnMsg);
        ...
        if (returnMsg != null) {
            CQ.sendGroupMsg(fromGroup, returnMsg);
        }
    ...
    public int groupMemberIncrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        // 这里处理消息
        CQ.sendGroupMsg(fromGroup, "欢迎" + CC.at(beingOperateQQ) + "加入MIST动漫社！进群请先按照年级-专业-名字的格式修改自己的群名片哦！");
        return MSG_IGNORE;
    }
